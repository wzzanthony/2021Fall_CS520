...
import random
import queue
from maze_agent_8 import Cell, Maze
import numpy as np
...
class RepeatedForwardAStar:
    ...
    def search(self, start_cell: Cell, only_bump=True, smart_restart=False):
        ...
        while not find_target:
            goal_cell_position_list = self.discovered_maze.find_next_goal(current_cell.get_position())
            count_Astar += 1
            if len(goal_cell_position_list) == 1:
                goal_cell_position = goal_cell_position_list[0]
                ...
                path = []
                for each_position in goal_cell_position_list:
                    goal_cell = Cell(each_position)
                    astar = AStar(self.discovered_maze, self.heuristic)
                    new_path = astar.search(current_cell, goal_cell)
                    if len(new_path)!=0:
                        if len(path) == 0 or len(new_path) < len(path):
                            path = new_path
                if len(path)==0:
                    # goal_cell_position = goal_cell_position_list[random.randint(0, len(goal_cell_position_list) - 1)]
                    for each_cell_position in goal_cell_position_list:
                        self.discovered_maze.update_poss(each_cell_position, True)
                    continue
            ...
            #     return moved_path
            if index_of_first_obstacle is None:
                status, test_times=self.maze.get_target(valid_path[-1].get_position())
                if status:
                    find_target = True
                    print(count_Astar)
                    return count_Astar, count_path_len, count_exam+test_times-1
