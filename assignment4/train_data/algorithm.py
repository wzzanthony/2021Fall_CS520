...
class RepeatedForwardAStar:
    ...
    def find_forward(self, path, index):
        # 上下左右
        path_len=len(path)
        if index==path_len-1:
            return [0,0,0,0]
        else:
            x1,y1=path[index].get_position()
            x2,y2=path[index+1].get_position()
            if x1-1==x2:
                return[1,0,0,0]
            elif x1+1==x2:
                return[0,1,0,0]
    ...
    def search(self, start_cell: Cell, goal_cell: Cell, only_bump=False, smart_restart=False, find_hallway=False):
        ...
        while current_cell != goal_cell:
            ...
            for index, cell_info in enumerate(path):
                count_train_data+=1
                train_data[str(count_train_data)]={}
                train_data[str(count_train_data)]["maze"]=copy.deepcopy(self.discovered_maze.get_maze())
                train_data[str(count_train_data)]["agent_position"]=cell_info.get_position()
                train_data[str(count_train_data)]["forward"]=self.find_forward(path, index)
