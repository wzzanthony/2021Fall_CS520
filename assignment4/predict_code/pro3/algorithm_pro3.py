import math
import queue
from maze_pro3 import Cell, Maze
import numpy as np
import torch
...
class RepeatedForwardAStar:
    ...
    def search(self, start_cell: Cell, model, max_path, only_bump=True, smart_restart=False):
        '''
        ...
        while not find_target:
            ...
            valid_path=[current_cell]
            while current_cell.get_position()!=goal_cell.get_position():
                # print(current_cell,"+", goal_cell)
                count_total_path+=1
                # create input data
                current_maze = self.discovered_maze.data
                current_agent_maze = [[0 for i in range(self.discovered_maze.width)] for j in
                                      range(self.discovered_maze.width)]
                cx, cy = current_cell.get_position()
                terrain = self.discovered_maze.maze_terrain[cx][cy]
                if terrain=="flat":
                    self.discovered_maze.find_target_prob[cx][cy]=0.8
                elif terrain=="hilly":
                    self.discovered_maze.find_target_prob[cx][cy] = 0.5
                elif terrain=="forest":
                    self.discovered_maze.find_target_prob[cx][cy] = 0.2
                gx,gy=goal_cell_position
                current_agent_maze[cx][cy] = 1
                current_agent_maze[gx][gy]=10
                ...
                # create next
                if self.maze.is_obstacle(cp[1][0], cp[2][0]):
                    self.discovered_maze.set_obstacle(cp[1][0], cp[2][0])
                    self.discovered_maze.update_poss((cp[1][0], cp[2][0]))
                    if (cp[1][0], cp[2][0])==goal_cell.get_position():
                        break
                    else:
                        continue
                else:
                    current_cell=Cell((cp[1][0], cp[2][0]))
                    valid_path=[current_cell]
                if count_total_path>max_path:
                    return False
            ...
            if smart_restart:
                ...
                if count_total_path%500==0:
                    print(count_total_path)
