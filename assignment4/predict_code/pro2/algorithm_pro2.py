...
from pro2.maze_for_ori_pro2 import Maze, Cell
...
def get_punish_list(current_position, punishment):
    cd, cx,cy=current_position[0][0], current_position[1][0], current_position[2][0]
    punish_list=[]
...
class SensingRepeatedForwardAStar:
    def __init__(self, maze: Maze, heuristic=None):
        """
    ...
    def check_hallway(self,valid_path):
        add_cells=[]
        new_valid_path=valid_path.copy()
        if len(valid_path)<2:
            return add_cells
        while True:
            current_cell=new_valid_path[-1]
            current_x, current_y=current_cell.get_position()
            father_cell=new_valid_path[-2]
            # if self.discovered_maze.maze[current_x][current_y].num_unconfirmed_neighbours==0:
            children_list=self.discovered_maze.generate_children(current_cell)
            if len(children_list)==2:
                children_list.remove(father_cell)
                new_child=children_list[0]
                nc_x, nc_y=new_child.get_position()
                if self.maze.is_obstacle(nc_x,nc_y):
                    self.initialize_visit_cell([nc_x,nc_y], "block")
                else:
                    self.initialize_visit_cell([nc_x,nc_y], "empty")
                    new_valid_path.append(new_child)
                    add_cells.append(new_child)
                    self.cell_processed.add((nc_x,nc_y))
            else:
                break
            # else:
            #     break
    ...
    def initialize_not_visit_cell(self, position, status="empty"):
        current_cell = self.discovered_maze.maze[position[0]][position[1]]
        if status == "empty":
            if not current_cell.is_visited():
                if self.discovered_maze.data[position[0]][position[1]] == "1":
    ...
    def search(self, start_cell: Cell, goal_cell: Cell, model, count_total, know_four_neighbours=False, use_infer_method=False, smart_restart=False, infer_more=False, refer_hallway=False):
        '''
        ...
        punishment=[[1 for i in range(self.discovered_maze.width)] for j in range(self.discovered_maze.width)]
        ...
        for i in range(count_total):
            ...
            # create input data
            current_maze=self.discovered_maze.data
            current_agent_maze=[[0 for i in range(self.discovered_maze.width)] for j in range(self.discovered_maze.width)]
            cx,cy=current_cell.get_position()
            current_agent_maze[cx][cy]=1
            input_data=torch.tensor([current_agent_maze, current_maze], dtype=torch.float).unsqueeze(0)

            # do predict
            output_data=model(input_data)
            output_data = torch.softmax(output_data, dim=1) + 1
            cp, punishment=move_forward(output_data, [[0], [cx], [cy]], self.discovered_maze.data, punishment)
            # create path
            path=[current_cell, Cell((cp[1][0], cp[2][0]))]
            ...
            if not know_four_neighbours:
                ...
                if infer_more:
                    ...
                    if refer_hallway:
                        add_path=self.check_hallway(valid_path)




                        # infer_valid_path=valid_path.copy()
                        infer_valid_path.extend(add_path)
