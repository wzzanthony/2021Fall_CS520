...
from algorithm_nh import SensingRepeatedForwardAStar, AStar
from algorithm import AStar as A
from maze_nh import Maze, Cell
...
def test(dim: int, num_experiment=NUM_EXPERIMENT):
    ...
    data_time_agent5 = [0.0] * len(PS)


    data_path_agent5 = [0.0] * len(PS)


    final_path_agent5 = [0.0] * len(PS)


    replan_time_agent5 = [0] * len(PS)
    ...
    for index_p, p in enumerate(PS):
        ...
        for random_seed in tqdm(range(num_experiment)):
            ...
            start_cell = Cell((0, 0))
            AStar_search = A(maze, euclidean_heuristic)
            ...
            et = time.time() - st
            data_time_agent5[index_p] += et
            data_path_agent5[index_p] += len(path_infer)
            AStar_search = A(sensing.discovered_maze, euclidean_heuristic)
            AStar_search_path = AStar_search.search(start_cell, goal_cell)
            final_path_agent5[index_p] += len(AStar_search_path)
            replan_time_agent5[index_p]+=sensing.replan_time



        if count != 0:

            data_time_agent5[index_p] = data_time_agent5[index_p] / count


            data_path_agent5[index_p] = data_path_agent5[index_p] / count


            final_path_agent5[index_p] = final_path_agent5[index_p] / count


            replan_time_agent5[index_p] = replan_time_agent5[index_p] / count

    return data_time_agent5,data_path_agent5,final_path_agent5,replan_time_agent5


if __name__ == '__main__':
    current_file_path=os.path.dirname(os.path.abspath(__file__))
    new_dir=os.path.join(current_file_path, "data3")
    ...
    save = { "data_path_agent5": b,
            "data_time_agent5": a,
            "final_path_agent5": c,
            "replan_time_agent5": d}
