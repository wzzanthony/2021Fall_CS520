...
class RepeatedForwardAStar:
    ...
    def search(self, start_cell: Cell, goal_cell: Cell, only_bump=False, smart_restart=False):
        '''
...
class BFS:
    ...
    def search(self, start_cell: Cell, goal_cell: Cell):
        # user a normal queue, select each cell among the neighbours to explore, first in
        fringe = queue.Queue()
        start_cell.set_father_node(None)
        fringe.put(start_cell)
        ...
        while not fringe.empty():
            ...
            # Generate the children of n (neighbors believed or known to be unoccupied)
            for child_cell in self.maze.generate_children(current_cell, goal_cell):
                if child_cell not in closed_list:
                    fringe.put(child_cell)
        # no path found
        return []
...
# Repeated Forward BFS
class RepeatedForwardBFS:
    def __init__(self, maze: Maze):
        """Initialize, accept a maze and the heuristic function"""
        self.maze = maze
        # the Discovered Gridworld
    ...
    def smart_find_restart_cell(self, moved_path: list) -> Cell:
        """Find the first cell that are not in the hallway"""
        # stats = []
        for index in range(-1, -len(moved_path) - 1, -1):
            cell = moved_path[index]
            x, y = cell.get_position()
            num_neighbours = 0
            num_obstacles = 0
            for dx, dy in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                nx, ny = x + dx, y + dy
                # neighbour is valid
                if self.discovered_maze.position_is_valid(nx, ny):
                    num_neighbours += 1
                    #  it's obstacle
                    if self.discovered_maze.is_obstacle(nx, ny):
                        num_obstacles += 1
    ...
    def search(self, start_cell: Cell, goal_cell: Cell, only_bump=False, smart_restart=False):
        ...
        # repeat until reach the goal
        while current_cell != goal_cell:
            # search the path in the current percepted maze using BFS!
            bfs = BFS(self.discovered_maze)
            path = bfs.search(current_cell, goal_cell)
            # print('Current at {}, path: {}'.format(current_cell, path))
            # could not find a path
            if not path:
                return []
